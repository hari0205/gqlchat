...
import { createUserResolver, deleteUserResolver, loginResolver, updateUserResolver } from "../services/user/user-services"
    ...
    Subscription: {
        messageSent: {
            subscribe: (_parent: any, _args: any, ctx: any, _: any) => {
                return ctx.pubsub.asyncIterator(["messageSent"])
            },
            resolve: (payload: any) => formatResponse(payload.newmessage)
        }
    }
}